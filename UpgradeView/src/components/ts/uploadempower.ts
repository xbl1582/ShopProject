...
export default defineComponent({
    ...
    watch:{
       async fileList(newvalue,oldvalue){
     //请求
        for(var i=0;i<newvalue.length;i++){
        await axios({
                    method: 'get',
                    url: '/empower/selectlic',
                    params: {
                    lic:this.fileList[i].name

                    }
                }).then((res: { data: { data: any; count: any; }; }) => {
                   
                    this.replacelist=res.data.data;
    ...
    methods: {
        ...
        submitUpload() {
            ...
            for(var i=0;i<this.replacelist.length;i++){
                licname+=this.replacelist[i].lic+","
                
            }

            ElMessageBox.confirm(
                '授权文件('+licname+')已经存在,是否替换',
                '提示',
                {
      ...
      */
        myUploadFile(data: { file: any; }) {
            ...
            axios.post('/empower/multiUpload', formData, {
                headers: {
